...
type TrackedKey = { id: string; keyId: string; label: string; projectId: string | null };
...
export async function getDashboardData(keys: TrackedKey[], rangeDays: number): Promise<DashboardData> {
  if (keys.length === 0) return emptyDashboard(rangeDays);
  if (getRuntimeEnv().ARGUS_DEMO_MODE === "true") return demoDashboard(keys, rangeDays);
  ...
  const start = now - rangeDays * 86_400;
  const previousStart = start - rangeDays * 86_400;
  ...
  const previousCostParams = new URLSearchParams({ start_time: String(previousStart), end_time: String(start), bucket_width: "1d", limit: String(Math.min(rangeDays + 1, 180)) });
  appendList(previousCostParams, "api_key_ids", keyIds);
  const [usageSettled, costSettled] = await Promise.all([
    Promise.allSettled(usageRequests),
    Promise.allSettled([fetchAll("costs", costParams, adminKey), fetchAll("costs", previousCostParams, adminKey)]),
  ]);
...
function aggregate(
  ...
  for (const group of usage) {
    for (const bucket of group.buckets) {
      const day = days.get(Math.floor(bucket.start_time / 86_400) * 86_400);
  ...
  const previousSpend = previousCostBuckets.flatMap((bucket) => bucket.results).reduce((sum, result) => {
...
function emptyDashboard(rangeDays: number): DashboardData {
  return {
    source: "empty", generatedAt: Math.floor(Date.now() / 1000), rangeDays,
...
function demoDashboard(keys: TrackedKey[], rangeDays: number): DashboardData {
  const now = Math.floor(Date.now() / 1000);
  ...
  return {
    source: "demo", generatedAt: now, rangeDays,
